...
*  ===========
*
*     RECURSIVE SUBROUTINE DTRTRM_CNT(SIDE, UPLO, TRANSV, DIAGT, DIAGV,
*    $                        N, ALPHA, T, LDT, V, LDV)
*
*        .. Scalar Arguments ..
*        INTEGER           N, LDT, LDV
...
c     Cost: 2/21 * (3n^3 + 7n^2 - 10) + 2
      RECURSIVE SUBROUTINE DTRTRM_CNT(SIDE, UPLO, TRANSV, DIAGT, DIAGV,
     $                        N, ALPHA, T, LDT, V, LDV, CNT)
*
*        .. Scalar Arguments ..
         INTEGER           N, LDT, LDV
         CHARACTER         SIDE, UPLO, TRANSV, DIAGT, DIAGV
         DOUBLE PRECISION  ALPHA, CNT
*        ..
*        .. Array Arguments ..
         DOUBLE PRECISION  T(LDT,*), V(LDV,*)
*        ..
...
*        .. Local Scalars ..
         ...
         DOUBLE PRECISION  TMPA, TMPB, TMPC
*        ..
*        .. Local Parameters ..
...
*
     ...
     $                     N-K, ALPHA, V(K+1, K+1), LDV, T(1, K+1), LDT)
                  TMPA = DBLE(K) ! m
                  TMPB = DBLE(N) - DBLE(K) ! n
                  TMPC = TMPA*TMPB*TMPB ! mn^2
                  CNT = CNT + TMPC
*
...
*
     ...
     $                     V(K+1, 1), LDV, ONE, T(1, K+1), LDT)
                  ! m,n same as above
                  TMPC = TMPA*TMPA*TMPB + 2.0*TMPA*TMPB ! m^2 n + 2mn
                  CNT = CNT + TMPC
               ELSE
*
...
*
     ...
     $                     V(1, K+1), LDV, ONE, T(1, K+1), LDT)
                  ! m^2 n + 2mn
                  TMPC = TMPA*TMPA*TMPB + 2.0*TMPA*TMPB
                  CNT = CNT + TMPC
               END IF
            ELSE
*
...
*
     ...
     $                     LDT, V(K+1, 1), LDV, ONE, T(1, K+1), LDT)
                  ! tmpc = mn^2 + 2*mn
                  TMPC = TMPA*TMPB*TMPB + 2.0*TMPA*TMPB
                  CNT = CNT + TMPC
               ELSE
*
...
*
         CALL DTRTRM_CNT(SIDE, UPLO, TRANSV, DIAGT, DIAGV, K, ALPHA,
     $         T, LDT, V, LDV, CNT)
*
*        Compute T_{22} recursively
*
         CALL DTRTRM_CNT(SIDE, UPLO, TRANSV, DIAGT, DIAGV, N-K, ALPHA,
     $         T(K+1, K+1), LDT, V(K+1, K+1), LDV, CNT)
