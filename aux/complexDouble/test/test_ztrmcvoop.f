     ...
     $      NEG_ONE=(-1.0D+0,0.0D+0))
         ...
         DO L = 1, 2 ! DIR(L): 'C','R'
            ...
            CALL ZTRMCVOOP(UPLOS(I), TRANS(J), DIAGS(K), N, ALPHA, A, N,
     $            X, INCX, BETA, Y, 1)
     ...
     $               J,")"
            ...
            END DO
            IF(TERMINATE) GOTO 10
            ! Now, we test the functionality by using dtrmvoop
            ! work = dconjg(x)
            IF (INCX.EQ.1) THEN
               DO II = 1, N
                  WORK(I,1) = DCONJG(X(I,1))
               END DO
            ELSE
               DO II = 1, N
                  WORK(1,I) = DCONJG(X(1,I))
               END DO
            END IF
            ! WORK = alpha*work
