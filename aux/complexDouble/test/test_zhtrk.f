     ...
     $            Cs(:,:), WORK(:,:)
         ...
         EXTERNAL          ZLANGE
         ! Intrinsic functions
         INTRINSIC SQRT,DCONJG,DBLE,DCMPLX
         ! Parameters
         COMPLEX*16 ONE, ZERO
         ...
         WRITE(*,*) "Provide a value for K to ZHTRK"
         READ(*,*) K
     ...
     $                  UNITS(O)
            ...
            CALL ZHTRK(UPLOTS(I), UPLOCS(J), TRANS(N), UNITS(O), K,
     $            ALPHA, T, K, BETA, C, K)
     ...
     $                       "(", II,",",IJ,")"
            ...
            DO IJ = 1, K
               ...
               TMP = TMP + WORK(II,IJ)*DCONJG(WORK(II,IJ))
            END DO
            END DO
            !
