...
int main(int argc, char **argv) {
    ...
    gettimeofday(&tp, NULL);
    double tmpTime = -((double)tp.tv_sec+(1.e-6)*tp.tv_usec);

    // Compute the T factor
    my_dlarft_ut_(&m, &k, Q, &ldq, tau);
    ...
    gettimeofday(&tp, NULL);
    tmpTime+=((double)tp.tv_sec+(1.e-6)*tp.tv_usec);
    elapsed_refL += tmpTime;
    ...
    dlacpy_(&uChar, &k, &k, work, &k, Q, &ldq, dummy);
    ...
    if (!errorsOnly && !timesOnly) {
        ...
        printf("T3:%10.10e %10.10e", norm_repres_1, norm_orth_1);
    } else {
        printf("T3:%10.10e:%10.10e", elapsed_refL, perform_refL);
    }
    printf("\n");
