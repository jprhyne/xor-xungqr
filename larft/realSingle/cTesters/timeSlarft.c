...
// ||Q**T * Q - I||
double computeOrthNorm(int m, int n, float *Q)
{
...
// ||As - QR|| / ||As||
double computeRepresNorm(int m, int n, float *Q, float *R, int ldr, float *As, int lda, float double)
{
    char aChar = 'a';
    char rChar = 'r';
    char uChar = 'u';
...
int main(int argc, char *argv[]) {
    ...
    int workQuery = -1;
    // float variables
    float *A, *As, *V, *Vs, *T, *work, *tau=NULL;
    double normA;
    ...
    As = (float *) malloc( lda * n * sizeof(float));
    V  = (float *) calloc( m * n, sizeof(float));
    Vs = (float *) malloc( m * n * sizeof(float));
    ...
    sgeqrf_(&m, &n, A, &lda, tau, work, &workQuery, &info );
    lwork = work[0];
    ...
    slarft_ref_(&fChar, &cChar, &m, &n, V, &m, tau, T, &n);
    ...
    slacpy_(&uChar, &m, &n, T, &n, V, &m, dummy);
    ...
    slacpy_(&aChar, &m, &n, Vs, &m, V, &m, dummy);
    ...
    elapsed_refL=-((double)tp.tv_sec+(1.e-6)*tp.tv_usec);
    // Call reference dlarft using optimized blas as the backend
    slarft_(&fChar, &cChar, &m, &n, V, &m, tau, T, &n, dummy, dummy);
    // grab the execution time
    gettimeofday(&tp, NULL);
    elapsed_refL+=((double)tp.tv_sec+(1.e-6)*tp.tv_usec);
    // Store this value 
    ...
    elapsed_refL=-((double)tp.tv_sec+(1.e-6)*tp.tv_usec);
    // Call reference dlarft using optimized blas as the backend
    my_slarft_rec_(&m, &n, V, &m, tau, T, &n);
    // grab the execution time
    gettimeofday(&tp, NULL);
    elapsed_refL+=((double)tp.tv_sec+(1.e-6)*tp.tv_usec);
    // Store this value 
