      SUBROUTINE TEST_ZTRMMOOP(M, N, ALPHA, BETA)
         ! Arguments
         INTEGER  M, N
            ...
            DO J = 1, 3 ! For each transposing of A
               DO K = 1, 3 ! For each transposing of B
                  DO L = 1, 2 ! For each of A being unit or non-unit
                     DO O = 1, 2 ! For each of A being upper or lower
                        ! Fill our matrices up with random numbers
                        CALL MY_RANDOM_NUMBER(MAXMN, MAXMN, A, MAXMN)
                        CALL MY_RANDOM_NUMBER(MAXMN, MAXMN, B, MAXMN)
                        CALL MY_RANDOM_NUMBER(MAXMN, MAXMN, C, MAXMN)
                        CALL MY_RANDOM_NUMBER(MAXMN, MAXMN, WORK, MAXMN)

                        ! Store A,B,C in As,Bs,Cs respectively
                        CALL ZLACPY('All', MAXMN, MAXMN, A, MAXMN, As,
     $                     MAXMN)
                        CALL ZLACPY('All', MAXMN, MAXMN, B, MAXMN, Bs,
     $                     MAXMN)
                        CALL ZLACPY('All', MAXMN, MAXMN, C, MAXMN, Cs,
     $                     MAXMN)

                        ! Compute our desired operation
                        CALL ZTRMMOOP(SIDES(I), UPLOS(O), TRANSAS(J),
     ...
     $                              "(",II,",",IJ,")"
                        ...
                        IF (TRANSBS(K).EQ.'N') THEN
                           CALL ZLACPY('ALL', MAXMN, MAXMN, Bs, MAXMN,
     $                        WORK, MAXMN)
                        ELSE IF (TRANSBS(K).EQ.'C') THEN
                           DO II = 1, MAXMN
                              DO IJ = 1, MAXMN
                                 WORK(II,IJ) = CONJG(Bs(IJ,II))
                              END DO
                           END DO
                        ELSE
                           DO II = 1, MAXMN
                           ...
                           END DO
                        END IF
                        CALL ZTRMM(SIDES(I), UPLOS(O), TRANSAS(J),
     $                     DIAGS(L), M, N, ALPHA, A, MAXMN, WORK, MAXMN)
                        ...
                        DO II = 1, M
                           DO IJ = 1, N
                              NORM_ACT = NORM_ACT +
     $                              WORK(II,IJ) * CONJG(WORK(II,IJ))
                           END DO
                        END DO
                        NORM_ACT = SQRT(NORM_ACT)
                        IF (NORM_ACT.NE.ZERO) THEN
                           NORM_F = NORM_F / NORM_ACT
                        END IF
                        ! Print out the flags used to allow for repeatability
                        WRITE(*,*) "Parameters to ZTRMMOOP"
...
10       DEALLOCATE(A)
      ...
      SUBROUTINE MY_RANDOM_NUMBER(M, N, Z, LDZ)
         ! Parameters
         INTEGER  M, N, LDZ
         COMPLEX*16  Z(LDZ,*)
         ! Local scalars
      ...
      SUBROUTINE MY_RANDOM_NUMBER_SCALAR(Z)
         ! Parameters
         COMPLEX*16  Z
         ! Local scalars
      ...
      SUBROUTINE ZTRMMOOP_TEST(M, N)
         ...
         CALL MY_RANDOM_NUMBER_SCALAR(ALPHA)
         CALL MY_RANDOM_NUMBER_SCALAR(BETA)
         CALL TEST_ZTRMMOOP(M, N, ALPHA, BETA)
      END SUBROUTINE
